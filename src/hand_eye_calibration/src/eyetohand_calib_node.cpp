...
const string  RESULT_FILENAME="result_eye_to_hand.xml";
...
int main(int argc, char** argv)
{
    ...
    ros::init(argc, argv, "eyetohand_calib_node");
    ros::start();
        if(argc != 5){
            cerr<<"Error"<<endl;
            cout<<"Usage rosrun Hand_Eye_calibration  eyetohand_calib_node number_of_images board_x_square_number board_y_square_number square_size"<<endl;
            return -1;
        }
    ...
    for(int i=0;i<HEmatrix_count;i++)
    {
        for (int j=0;j<HEmatrix_count;j++)
        {
                ...
                D[CDcount].transform=Y[i].transform*(Y[j].transform.inv());
    ...
    //寻找最好的结果
     ...
     fs<<"xtion"<<trans_result;
     fs.release(); 
     base_link2camera_link(trans_result);
     cout<<"kinect2"<<endl;
     base_link2rbg_optical_frame(trans_result);

     cout<<"endtoboard"<<endl;
     Mat end2board(4,4,CV_64F);

     cout<<Y[1].transform<<endl;
     cout<<E[1].transform<<endl;
     end2board = Y[1].transform.inv()*trans_result*E[1].transform;
     cout<<end2board<<endl;
