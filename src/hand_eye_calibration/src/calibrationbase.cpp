...
bool moveToPose(String jointgroup,geometry_msgs::Pose  target_pose)
{
    ...
    sleep(5);
    ROS_INFO("Visualizing plan 1 (pose goal) %s",success==moveit::planning_interface::MoveItErrorCode::SUCCESS?"":"FAILED");
    if(success){
      group.execute(my_plan);
    }
    sleep(5);
    ros::spinOnce();
    return success==moveit::planning_interface::MoveItErrorCode::SUCCESS;
}
...
// 使用关节值来控制机器人
bool movetoPoseJointstate(String jointgroup,String referenceFrame,Mat jointstate,int nJoint)
{
...
//      ROS_INFO("Visualizing plan 1 (pose goal) %s",success?"":"FAILED");
      ...
      moveit::planning_interface::MoveItErrorCode success = Jointgroup.plan(my_plan);
      ROS_INFO("Visualizing plan 1 (pose goal) %s",success==moveit::planning_interface::MoveItErrorCode::SUCCESS?"":"FAILED");
...
bool movetoPoseJointstate_fast(String jointgroup,String referenceFrame,Mat jointstate,int nJoint)
{
...
//the ar poseture is  wrong
 ...
 bool findPoseture(String end_frame,String baseframe,tf::StampedTransform *result)
 {
     tf::TransformListener listener;
     tf::StampedTransform transform;
     ros::spinOnce();
          try{
                listener.waitForTransform(baseframe,end_frame,  ros::Time(0), ros::Duration(2));
                listener.lookupTransform(baseframe,end_frame, ros::Time(0), transform);
              }
              catch (tf::TransformException ex){
                ROS_ERROR("%s",ex.what());
                ros::Duration(1.0).sleep();
...
void base_link2rbg_optical_frame(Mat transbase2camera_rbg_optical_frame)
{
Mat dcm=transbase2camera_rbg_optical_frame(Range(0,3),Range(0,3));
Mat quat =dcm2quat(dcm);
Mat t=findTFromTrans(transbase2camera_rbg_optical_frame);
...
qz=quat.at<double>(0,3);
qw=-quat.at<double>(0,0);
ROS_INFO("rosrun tf static_transform_publisher  %f  %f  %f  %f  %f  %f  %f    /base_link /camera_rgb_optical_frame 40",
    x,y,z,qx,qy,qz,qw);
...
Mat transrgb2cameralink(4,4,CV_64F);
transrgb2cameralink.at<double>(0,0)=0;
transrgb2cameralink.at<double>(0,1)=-1;
transrgb2cameralink.at<double>(0,2)=0;
transrgb2cameralink.at<double>(0,3)=0;

transrgb2cameralink.at<double>(1,0)=0;
transrgb2cameralink.at<double>(1,1)=0;
transrgb2cameralink.at<double>(1,2)=-1;
transrgb2cameralink.at<double>(1,3)=0;

transrgb2cameralink.at<double>(2,0)=1;
transrgb2cameralink.at<double>(2,1)=0;
transrgb2cameralink.at<double>(2,2)=0;
transrgb2cameralink.at<double>(2,3)=0;
...
Mat transbase2cameralink=transbase2camera_rbg_optical_frame*(transrgb2cameralink);
Mat dcm=transbase2cameralink(Range(0,3),Range(0,3));
Mat quat =dcm2quat(dcm);
Mat t=findTFromTrans(transbase2cameralink);
